...
module.exports = function(params, callback) {
  ...
  function downloadSchemas(from, parent) {
    extractRefs(from).forEach(function(url) {
      var test = isLocal(url, normalized_fakeroot),
          base = deref.util.getDocumentURI(!test ? deref.util.resolveURL(normalized_fakeroot, url) : url);
      ...
      if (test) {
        ...
        var file = path.join(parent, base);

        if (file.indexOf(schema_directory) === -1) {
          // TODO: im not sure about any strategy here...
          file = path.join(schema_directory, base.replace(/(\.\.?\/)+/g, ''));
        }
          ...
          req.get(url, function(res) {
            ...
            res.on('end', function() {
              try {
                ...
                next(e.message ? new Error('unable to JSON.parse from ' + url + ' (' + e.message + ')') : e);
              }
            });
  ...
  try {
    ...
    schemas.forEach(function(data) {
      ...
      downloadSchemas(data, schema_directory);
    });
