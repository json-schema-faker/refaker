...
module.exports = function(params, callback) {
  ...
  function downloadSchemas(uri, from, parent) {
        ...
        extractRefs(schema).forEach(function(ref) {
          ...
          if (url.indexOf(normalized_fakeroot) === 0) {
            ...
            if (fs.statSync(file).isFile()) {
              downloadSchemas(url, parseJSON(fs.readFileSync(file).toString(), file), path.dirname(file));
            }
          } else {
            _.then(function(next) {
              ...
              req.get(url, function(res) {
                ...
                res.on('end', function() {
                  try {
                    next(downloadSchemas(url, parseJSON(body, url), $.util.resolveURL(url, '../')));
                  } catch (e) {
                    next(e);
                  }
                });
    ...
    pushReference($.util.normalizeSchema(uri, from, pushReference));
  }
  ...
  try {
    ...
    schemas.forEach(function(data) {
      downloadSchemas(normalized_fakeroot, data, schema_directory);
    });
